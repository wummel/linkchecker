...
class Renderer(object):
    ...
    Most applications can use the higher-level L{bk.dns.message.Message}
    ...
    Typical use::
        ...
        r = bk.dns.renderer.Renderer(id=1, flags=0x80, max_size=512)
        r.add_question(qname, qtype, qclass)
        r.add_rrset(bk.dns.renderer.ANSWER, rrset_1)
        r.add_rrset(bk.dns.renderer.ANSWER, rrset_2)
        r.add_rrset(bk.dns.renderer.AUTHORITY, ns_rrset)
        r.add_edns(0, 0, 4096)
        r.add_rrset(bk.dns.renderer.ADDTIONAL, ad_rrset_1)
        r.add_rrset(bk.dns.renderer.ADDTIONAL, ad_rrset_2)
        r.write_header()
    ...
    @type section: int (bk.dns.renderer.QUESTION, bk.dns.renderer.ANSWER,
    bk.dns.renderer.AUTHORITY, or bk.dns.renderer.ADDITIONAL)
    ...
    def __init__(self, id=None, flags=0, max_size=65535, origin=None):
        ...
        then L{bk.dns.exception.TooBig} will be raised.
        @type max_size: int
        @param origin: the origin to use when rendering relative names
        @type origin: bk.dns.name.Namem or None.
        """
    ...
    def _set_section(self, section):
        ...
        @type section: int
        @raises bk.dns.exception.FormError: an attempt was made to set
    ...
    def add_question(self, qname, rdtype, rdclass=bk.dns.rdataclass.IN):
        """Add a question to the message.

        @param qname: the question name
        @type qname: bk.dns.name.Name
    ...
    def add_rdataset(self, section, name, rdataset, **kw):
        ...
        @type rdataset: bk.dns.rdataset.Rdataset object
        """
    ...
    def add_edns(self, edns, ednsflags, payload):
        ...
        before = self.output.tell()
        self.output.write(struct.pack('!BHHIH', 0, bk.dns.rdatatype.OPT, payload,
                                      ednsflags, 0))
        after = self.output.tell()
        if after >= self.max_size:
            self._rollback(before)
    ...
    def add_tsig(self, keyname, secret, fudge, id, tsig_error, other_data,
        ...
        (tsig_rdata, self.mac, ctx) = bk.dns.tsig.hmac_md5(s,
                                                        keyname,
                                                        secret,
                                                        int(time.time()),
                                                        fudge,
        ...
        self.output.write(struct.pack('!HHIH', bk.dns.rdatatype.TSIG,
                                      bk.dns.rdataclass.ANY, 0, 0))
