...
class UngetBufferFull(bk.dns.exception.DNSException):
...
class Tokenizer(object):
    ...
    def get(self, want_leading = False, want_comment = False):
        ...
        @raises bk.dns.exception.UnexpectedEnd: input ended prematurely
        @raises bk.dns.exception.SyntaxError: input was badly formed
        """
        ...
        while True:
            ...
            if c == '' or c in self.delimiters:
                if c == '' and self.quoting:
                    raise bk.dns.exception.UnexpectedEnd
                ...
                if c == '\\':
                    ...
                    raise bk.dns.exception.SyntaxError, 'newline in quoted string'
            elif c == '\\':
                #
        ...
        if token == '' and ttype != QUOTED_STRING:
            if self.multiline:
                raise bk.dns.exception.SyntaxError, 'unbalanced parentheses'
            ttype = EOF
        return (ttype, token)
    ...
    def get_int(self):
        ...
        if ttype != IDENTIFIER:
            raise bk.dns.exception.SyntaxError, 'expecting an identifier'
        if not value.isdigit():
            raise bk.dns.exception.SyntaxError, 'expecting an integer'
        return int(value)
    ...
    def get_string(self, origin=None):
        ...
        if ttype != IDENTIFIER and ttype != QUOTED_STRING:
            raise bk.dns.exception.SyntaxError, 'expecting a string'
        return t
    ...
    def get_name(self, origin=None):
        ...
        if ttype != IDENTIFIER:
            raise bk.dns.exception.SyntaxError, 'expecting an identifier'
        return bk.dns.name.from_text(t, origin)
