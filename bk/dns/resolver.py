...
@type default_resolver: bk.dns.resolver.Resolver object"""
...
class NXDOMAIN(bk.dns.exception.DNSException):
    """The query name does not exist."""
    pass

# The definition of the Timeout exception has moved from here to the
# bk.dns.exception module.  We keep bk.dns.resolver.Timeout defined for
# backwards compatibility.

Timeout = bk.dns.exception.Timeout

class NoAnswer(bk.dns.exception.DNSException):
    """The response did not contain an answer to the question."""
    pass

class NoNameservers(bk.dns.exception.DNSException):
...
class Answer(object):
    ...
    @type qname: bk.dns.name.Name object
    @ivar rdtype: The query type
    @type rdtype: int
    @ivar rdclass: The query class
    @type rdclass: int
    @ivar response: The response message
    @type response: bk.dns.message.Message object
    @ivar rrset: The answer
    @type rrset: bk.dns.rrset.RRset object
    ...
    def __init__(self, qname, rdtype, rdclass, response):
        ...
        for count in xrange(0, 15):
            try:
                ...
                if rdtype != bk.dns.rdatatype.CNAME:
                    try:
                        crrset = response.find_rrset(response.answer,
                                                     qname,
                                                     rdclass,
                                                     bk.dns.rdatatype.CNAME)
...
class Cache(object):
    ...
    def get(self, key):
        ...
        @param key: the key
        @type key: (bk.dns.name.Name, int, int) tuple whose values are the
        query name, rdtype, and rdclass.
        @rtype: bk.dns.resolver.Answer object or None
        """
    ...
    def put(self, key, value):
        ...
        @type value: bk.dns.resolver.Answer object
        """
    ...
    def flush(self, key=None):
        ...
        @type key: (bk.dns.name.Name, int, int) tuple or None
        """
...
class Resolver(object):
    ...
    @type domain: bk.dns.name.Name object
    ...
    @type search: list of bk.dns.name.Name objects
    ...
    @type cache: bk.dns.resolver.Cache object
    """
    ...
    def reset(self):
        ...
        self.domain = \
            bk.dns.name.Name(bk.dns.name.from_text(socket.gethostname())[1:])
        if len(self.domain) == 0:
            self.domain = bk.dns.name.root
        self.nameservers = []
        self.search = []
        self.port = 53
        self.timeout = 2.0
        ...
        try:
            for l in f:
                ...
                if tokens[0] == 'nameserver':
                    ...
                    self.domain = bk.dns.name.from_text(tokens[1])
                elif tokens[0] == 'search':
                    for suffix in tokens[1:]:
                        self.search.append(bk.dns.name.from_text(suffix))
        finally:
            if want_close:
                f.close()
    ...
    def _config_win32_domain(self, domain):
        ...
        self.domain = bk.dns.name.from_text(str(domain))
    ...
    def _config_win32_search(self, search):
        ...
        for s in search_list:
            if not s in self.search:
                self.search.append(bk.dns.name.from_text(s))
    ...
    def query(self, qname, rdtype=bk.dns.rdatatype.A, rdclass=bk.dns.rdataclass.IN,
              tcp=False):
        ...
        @type tcp: bool
        @rtype: bk.dns.resolver.Answer instance
        ...
        if isinstance(qname, str):
            qname = bk.dns.name.from_text(qname, None)
        ...
        if qname.is_absolute():
            ...
            if len(qname) > 1:
                qnames_to_try.append(qname.concatenate(bk.dns.name.root))
            if self.search:
        ...
        for qname in qnames_to_try:
            if self.cache:
                ...
                if answer:
                    return answer
            request = bk.dns.message.make_query(qname, rdtype, rdclass)
            ...
            while response is None:
                ...
                for nameserver in nameservers:
                    ...
                    try:
                        if tcp:
                            response = bk.dns.query.tcp(request, nameserver,
                                                     timeout, self.port)
                        else:
                            response = bk.dns.query.udp(request, nameserver,
                                                     timeout, self.port)
                    except socket.error:
                        #
                        ...
                        response = None
                        continue
                    except bk.dns.query.UnexpectedSource:
                        #
                        # Who knows?  Keep going.
                        #
                        response = None
                        continue
                    except bk.dns.exception.FormError:
                        #
                    ...
                    if rcode == bk.dns.rcode.NOERROR or \
                           rcode == bk.dns.rcode.NXDOMAIN:
                        break
                    response = None
            if response.rcode() == bk.dns.rcode.NXDOMAIN:
                continue
            all_nxdomain = False
            break
        if all_nxdomain:
    ...
    def use_edns(self, edns, ednsflags, payload):
        ...
        @param edns: The EDNS level to use.  The default is -1, no Ebk.dns.
        @type edns: int
...
def query(qname, rdtype=bk.dns.rdatatype.A, rdclass=bk.dns.rdataclass.IN,
          tcp=False):
    """Query nameservers to find the answer to the question.

    This is a convenience function that uses the default resolver
    object to make the query.
    @see: L{bk.dns.resolver.Resolver.query} for more information on the
    parameters."""
