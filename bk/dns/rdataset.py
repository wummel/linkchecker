...
class Rdataset(bk.dns.set.Set):
    ...
    bk.dns.rdatatype.NONE, but if the rdtype is bk.dns.rdatatype.SIG or
    bk.dns.rdatatype.RRSIG, then the covers value will be the rdata
    ...
    def __init__(self, rdclass, rdtype, covers=bk.dns.rdatatype.NONE):
    ...
    def add(self, rd, ttl=None):
        ...
        @param rd: The rdata
        @type rd: bk.dns.rdata.Rdata object
        @param ttl: The TTL
        @type ttl: int"""
        ...
        if not ttl is None:
            self.update_ttl(ttl)
        if self.rdtype == bk.dns.rdatatype.RRSIG or \
           self.rdtype == bk.dns.rdatatype.SIG:
            covers = rd.covers()
            if len(self) == 0 and self.covers == bk.dns.rdatatype.NONE:
                self.covers = covers
            elif self.covers != covers:
                raise DifferingCovers
        if bk.dns.rdatatype.is_singleton(rd.rdtype) and len(self) > 0:
            self.clear()
    ...
    def update(self, other):
        ...
        @type other: bk.dns.rdataset.Rdataset object"""
        self.update_ttl(other.ttl)
    ...
    def __repr__(self):
        if self.covers == 0:
            ...
            ctext = '(' + bk.dns.rdatatype.to_text(self.covers) + ')'
        return '<DNS ' + bk.dns.rdataclass.to_text(self.rdclass) + ' ' + \
               bk.dns.rdatatype.to_text(self.rdtype) + ctext + ' rdataset>'
    ...
    def to_text(self, name=None, origin=None, relativize=True,
        ...
        @see: L{bk.dns.name.Name.choose_relativity} for more information
        ...
        if len(self) == 0:
            ...
            print >> s, '%s%s%s %s' % (ntext, pad,
                                       bk.dns.rdataclass.to_text(rdclass),
                                       bk.dns.rdatatype.to_text(self.rdtype))
        else:
            for rd in self:
                print >> s, '%s%s%d %s %s %s' % \
                      (ntext, pad, self.ttl, bk.dns.rdataclass.to_text(rdclass),
                       bk.dns.rdatatype.to_text(self.rdtype),
...
def from_text_list(rdclass, rdtype, ttl, text_rdatas):
    ...
    @rtype: bk.dns.rdataset.Rdataset object
    """
    ...
    for t in text_rdatas:
        rd = bk.dns.rdata.from_text(r.rdclass, r.rdtype, t)
        r.add(rd)
    return r
