...
@type root: bk.dns.name.Name object
@var empty: The empty DNS name.
@type empty: bk.dns.name.Name object
"""
...
class Name(object):
    ...
    The bk.dns.name.Name class represents a DNS name as a tuple of labels.
    ...
    def fullcompare(self, other):
        ...
        and is one of: bk.dns.name.NAMERELN_NONE,
        bk.dns.name.NAMERELN_SUPERDOMAIN, bk.dns.name.NAMERELN_SUBDOMAIN,
        bk.dns.name.NAMERELN_EQUAL, or bk.dns.name.NAMERELN_COMMONANCESTOR
    ...
    def split(self, depth):
        ...
        l = len(self.labels)
        if depth == 0:
            return (self, bk.dns.name.empty)
        elif depth == l:
            return (bk.dns.name.empty, self)
...
def from_wire(message, current):
    ...
    of the message
    @type current: int
    @raises bk.dns.name.BadPointer: a compression pointer did not point backwards
    in the message
    @raises bk.dns.name.BadLabelType: an invalid label type was encountered.
    @returns: a tuple consisting of the name that was read and the number
    of bytes of the wire format message which were consumed reading it
    @rtype: (bk.dns.name.Name object, int) tuple
    """
