...
class BadTime(bk.dns.exception.DNSException):
    """Raised if the current time is not within the TSIG's validity time."""
    pass

class BadSignature(bk.dns.exception.DNSException):
    """Raised if the TSIG signature fails to verify."""
    pass

_alg_name = bk.dns.name.from_text('HMAC-MD5.SIG-ALG.REG.INT.').to_digestable()
...
def hmac_md5(wire, keyname, secret, time, fudge, original_id, error,
    ...
    if first:
        ...
        ctx.update(struct.pack('!H', bk.dns.rdataclass.ANY))
...
def validate(wire, keyname, secret, now, request_mac, tsig_start, tsig_rdata,
    ...
    current = tsig_rdata
    (aname, used) = bk.dns.name.from_wire(wire, current)
