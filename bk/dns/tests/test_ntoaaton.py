...
class TestNtoAAtoN (unittest.TestCase):
    ...
    def test_aton1(self):
        a = bk.dns.ipv6.inet_aton('::')
        self.assertEqual(a, '\x00' * 16)

    def test_aton2(self):
        a = bk.dns.ipv6.inet_aton('::1')
        self.assertEqual(a, '\x00' * 15 + '\x01')

    def test_aton3(self):
        a = bk.dns.ipv6.inet_aton('::10.0.0.1')
        self.assertEqual(a, '\x00' * 12 + '\x0a\x00\x00\x01')

    def test_aton4(self):
        a = bk.dns.ipv6.inet_aton('abcd::dcba')
        self.assertEqual(a, '\xab\xcd' + '\x00' * 12 + '\xdc\xba')

    def test_aton5(self):
        a = bk.dns.ipv6.inet_aton('1:2:3:4:5:6:7:8')
        self.assertEqual(a,
    ...
    def test_bad_aton2(self):
        def bad():
            a = bk.dns.ipv6.inet_aton('abcd::dcba::1')
        self.assertRaises(bk.dns.exception.SyntaxError, bad)

    def test_bad_aton3(self):
        def bad():
            a = bk.dns.ipv6.inet_aton('1:2:3:4:5:6:7:8:9')
        self.assertRaises(bk.dns.exception.SyntaxError, bad)
    ...
    def test_bad_ntoa1(self):
        def bad():
            a = bk.dns.ipv6.inet_ntoa('')
        self.assertRaises(ValueError, bad)

    def test_bad_ntoa2(self):
        def bad():
            a = bk.dns.ipv6.inet_ntoa('\x00' * 17)
        self.assertRaises(ValueError, bad)
