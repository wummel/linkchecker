...
class TestZone (unittest.TestCase):
    ...
    def testFromFile1(self):
        z = bk.dns.zone.from_file(fname('example'), 'example')
        ok = False
        try:
    ...
    def testFromText(self):
        z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        f = StringIO.StringIO()
        names = z.nodes.keys()
        names.sort()
        for n in names:
    ...
    def testTorture1(self):
        ...
        #
        f = StringIO.StringIO()
        o = bk.dns.name.from_text('example.')
        z = bk.dns.zone.from_file(fname('example'), o)
        for (name, node) in z.iteritems():
            for rds in node:
                for rd in rds:
                    ...
                    wire = f.getvalue()
                    rd2 = bk.dns.rdata.from_wire(rds.rdclass, rds.rdtype,
                                              wire, 0, len(wire),
                                              origin = o)
                    self.assertEqual(rd, rd2)

    def testEqual(self):
        z1 = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        z2 = bk.dns.zone.from_text(example_text_output, 'example.',
                                relativize=True)
        self.assertEqual(z1, z2)

    def testNotEqual1(self):
        z1 = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        z2 = bk.dns.zone.from_text(something_quite_similar, 'example.',
                                relativize=True)
        self.assertNotEqual(z1, z2)

    def testNotEqual2(self):
        z1 = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        z2 = bk.dns.zone.from_text(something_different, 'example.',
                                relativize=True)
        self.assertNotEqual(z1, z2)

    def testNotEqual3(self):
        z1 = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        z2 = bk.dns.zone.from_text(something_different, 'example2.',
                                relativize=True)
        self.assertNotEqual(z1, z2)
    ...
    def testFindRdataset1(self):
        ...
        exrds = bk.dns.rdataset.from_text('IN', 'SOA', 300, 'foo bar 1 2 3 4 5')
    ...
    def testFindRRset1(self):
        ...
        exrrs = bk.dns.rrset.from_text('@', 300, 'IN', 'SOA', 'foo bar 1 2 3 4 5')
    ...
    def testReplaceRdataset1(self):
        z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        rdataset = bk.dns.rdataset.from_text('in', 'ns', 300, 'ns3', 'ns4')
    ...
    def testReplaceRdataset2(self):
        z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        rdataset = bk.dns.rdataset.from_text('in', 'txt', 300, '"foo"')
        z.replace_rdataset('@', rdataset)
    ...
    def testNodeFindRdataset1(self):
        z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        node = z['@']
        rds = node.find_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.SOA)
        exrds = bk.dns.rdataset.from_text('IN', 'SOA', 300, 'foo bar 1 2 3 4 5')
        self.assertEqual(rds, exrds)
    ...
    def testNodeFindRdataset2(self):
        def bad():
            z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
            node = z['@']
            rds = node.find_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.LOC)
        self.assertRaises(KeyError, bad)

    def testNodeGetRdataset1(self):
        z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        node = z['@']
        rds = node.get_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.SOA)
        exrds = bk.dns.rdataset.from_text('IN', 'SOA', 300, 'foo bar 1 2 3 4 5')
        self.assertEqual(rds, exrds)
    ...
    def testNodeGetRdataset2(self):
        z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        node = z['@']
        rds = node.get_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.LOC)
        self.assertEqual(rds, None)

    def testNodeDeleteRdataset1(self):
        z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        node = z['@']
        rds = node.delete_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.SOA)
        rds = node.get_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.SOA)
        self.assertEqual(rds, None)
    ...
    def testNodeDeleteRdataset2(self):
        z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        node = z['@']
        rds = node.delete_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.LOC)
        rds = node.get_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.LOC)
        self.assertEqual(rds, None)
    ...
    def testIterateRdatasets(self):
        ...
        ns.sort()
        self.assertEqual(ns, [bk.dns.name.from_text('ns1', None),
                               bk.dns.name.from_text('ns2', None)])

    def testIterateRdatas(self):
        z = bk.dns.zone.from_text(example_text, 'example.', relativize=True)
        l = list(z.iterate_rdatas('A'))
        l.sort()
        exl = [(bk.dns.name.from_text('ns1', None),
                3600,
                bk.dns.rdata.from_text(bk.dns.rdataclass.IN, bk.dns.rdatatype.A,
                                    '10.0.0.1')),
               (bk.dns.name.from_text('ns2', None),
                3600,
                bk.dns.rdata.from_text(bk.dns.rdataclass.IN, bk.dns.rdatatype.A,
    ...
    def testTTLs(self):
        z = bk.dns.zone.from_text(ttl_example_text, 'example.', relativize=True)
        n = z['@']
        rds = n.get_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.SOA)
        self.assertEqual(rds.ttl, 3600)
        n = z['ns1']
        rds = n.get_rdataset(bk.dns.rdataclass.IN, bk.dns.rdatatype.A)
    ...
    def testNoSOA(self):
        def bad():
            z = bk.dns.zone.from_text(no_soa_text, 'example.',
                                   relativize=True)
        self.assertRaises(bk.dns.zone.NoSOA, bad)

    def testNoNS(self):
        def bad():
            z = bk.dns.zone.from_text(no_ns_text, 'example.',
                                   relativize=True)
        self.assertRaises(bk.dns.zone.NoNS, bad)

    def testInclude(self):
        z1 = bk.dns.zone.from_text(include_text, 'example.', relativize=True,
                                   allow_include=True)
        z2 = bk.dns.zone.from_file(fname('example'), 'example.',
                                   relativize=True)
        self.assertEqual(z1, z2)

    def testBadDirective(self):
        def bad():
            z = bk.dns.zone.from_text(bad_directive_text, 'example.',
                                      relativize=True)
        self.assertRaises(bk.dns.exception.SyntaxError, bad)
