...
class TestUpdate (unittest.TestCase):
    ...
    def test_to_wire1(self):
        update = bk.dns.update.Update('example')
        update.id = 1
        update.present('foo')
    ...
    def test_to_wire2(self):
        ...
        update.absent('blaz2', 'a')
        update.replace('foo', 300, 'a', '10.0.0.1', '10.0.0.2')
        update.add('bar', 300, bk.dns.rdata.from_text(1, 1, '10.0.0.3'))
        update.delete('bar', 'a', '10.0.0.4')
        update.delete('blaz','a')
        update.delete('blaz2')
        self.assertEqual(update.to_wire(), goodwire)
    ...
    def test_to_wire3(self):
        ...
        update.absent('blaz2', 'a')
        update.replace('foo', 300, 'a', '10.0.0.1', '10.0.0.2')
        update.add('bar', bk.dns.rdataset.from_text(1, 1, 300, '10.0.0.3'))
        update.delete('bar', 'a', '10.0.0.4')
        update.delete('blaz','a')
        update.delete('blaz2')
        self.assertEqual(update.to_wire(), goodwire)
    ...
    def test_from_text1(self):
        update = bk.dns.message.from_text(update_text)
        w = update.to_wire(origin=bk.dns.name.from_text('example'),
                           want_shuffle=False)
