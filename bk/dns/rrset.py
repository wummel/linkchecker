...
class RRset(bk.dns.rdataset.Rdataset):
    ...
    def __init__(self, name, rdclass, rdtype, covers=bk.dns.rdatatype.NONE,
                 deleting=None):
        """Create a new RRset."""
    ...
    def __repr__(self):
        ...
        if not self.deleting is None:
            dtext = ' delete=' + bk.dns.rdataclass.to_text(self.deleting)
        else:
            dtext = ''
        return '<DNS ' + str(self.name) + ' ' + \
               bk.dns.rdataclass.to_text(self.rdclass) + ' ' + \
               bk.dns.rdatatype.to_text(self.rdtype) + ctext + dtext + ' RRset>'
