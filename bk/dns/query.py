...
class UnexpectedSource(bk.dns.exception.DNSException):
    """Raised if a query response comes from an unexpected address or port."""
    pass

class BadResponse(bk.dns.exception.FormError):
...
def _wait_for(ir, iw, ix, expiration):
    if expiration is None:
        ...
        if timeout <= 0.0:
            raise bk.dns.exception.Timeout
    if timeout is None:
...
def udp(q, where, timeout=None, port=53, af=socket.AF_INET):
    ...
    r = bk.dns.message.from_wire(wire, keyring=q.keyring, request_mac=q.mac)
...
def tcp(q, where, timeout=None, port=53, af=socket.AF_INET):
    ...
    @param q: the query
    @type q: bk.dns.message.Message object
    @param where: where to send the message
    @type where: string
    @param timeout: The number of seconds to wait before the query times out.
    If None, the default, wait forever.
...
def xfr(where, zone, rdtype=bk.dns.rdatatype.AXFR, rdclass=bk.dns.rdataclass.IN,
    ...
    @type zone: bk.dns.name.Name object or string
    @param rdtype: The type of zone transfer.  The default is
    bk.dns.rdatatype.AXFR.
    ...
    @type keyname: bk.dns.name.Name object or string
    ...
    take.
    @type lifetime: float
    @rtype: generator of bk.dns.message.Message objects."""

    if isinstance(zone, str):
        zone = bk.dns.name.from_text(zone)
    q = bk.dns.message.make_query(zone, rdtype, rdclass)
    ...
    while not done:
        ...
        if not seen_soa:
            ...
            rrset = r.answer[0]
            if rrset.rdtype != bk.dns.rdatatype.SOA:
                raise bk.dns.exception.FormError
            seen_soa = True
            if len(r.answer) > 1 and r.answer[-1].name == oname:
                ...
                if rrset.rdtype == bk.dns.rdatatype.SOA:
                    if q.keyring and not r.had_tsig:
                        raise bk.dns.exception.FormError, "missing TSIG"
                    done = True
