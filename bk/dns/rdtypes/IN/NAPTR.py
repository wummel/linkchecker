...
class NAPTR(bk.dns.rdata.Rdata):
    """NAPTR record
    ...
    @type replacement: bk.dns.name.Name object
    @see: RFC 3403"""
    ...
    def to_text(self, origin=None, relativize=True, **kw):
        ...
        return '%d %d "%s" "%s" "%s" %s' % \
               (self.order, self.preference,
                bk.dns.rdata._escapify(self.flags),
                bk.dns.rdata._escapify(self.service),
                bk.dns.rdata._escapify(self.regexp),
                self.replacement)
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin = None):
        ...
        for i in xrange(3):
            ...
            rdlen -= l
            strings.append(s)
        (replacement, cused) = bk.dns.name.from_wire(wire[: current + rdlen],
                                                  current)
        if cused != rdlen:
            raise bk.dns.exception.FormError
