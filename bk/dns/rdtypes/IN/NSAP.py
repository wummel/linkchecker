...
class NSAP(bk.dns.rdata.Rdata):
    ...
    def from_text(cls, rdclass, rdtype, tok, origin = None, relativize = True):
        ...
        t = tok.get_eol()
        if address[0:2] != '0x':
            raise bk.dns.exception.SyntaxError, 'string does not start with 0x'
        address = address[2:].replace('.', '')
        if len(address) % 2 != 0:
            raise bk.dns.exception.SyntaxError, 'hexstring has odd length'
