...
class PX(bk.dns.rdata.Rdata):
    """PX record.

    @ivar preference: the preference value
    @type preference: int
    @ivar map822: the map822 name
    @type map822: bk.dns.name.Name object
    @ivar mapx400: the mapx400 name
    @type mapx400: bk.dns.name.Name object
    @see: RFC 2163"""
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin = None):
        ...
        current += 2
        rdlen -= 2
        (map822, cused) = bk.dns.name.from_wire(wire[: current + rdlen],
                                               current)
        if cused > rdlen:
        ...
        (mapx400, cused) = bk.dns.name.from_wire(wire[: current + rdlen],
                                              current)
        if cused != rdlen:
            raise bk.dns.exception.FormError
        if not origin is None:
