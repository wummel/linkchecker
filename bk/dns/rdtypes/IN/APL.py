...
class APLItem(object):
    ...
    def to_wire(self, file):
        if self.family == 1:
            address = bk.dns.inet.inet_pton(bk.dns.inet.AF_INET, self.address)
        elif self.family == 2:
            address = bk.dns.inet.inet_pton(bk.dns.inet.AF_INET6, self.address)
        else:
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin = None):
        ...
        while 1:
            ...
            if header[0] == 1:
                if l < 4:
                    address += '\x00' * (4 - l)
                address = bk.dns.inet.inet_ntop(bk.dns.inet.AF_INET, address)
            elif header[0] == 2:
                if l < 16:
                    address += '\x00' * (16 - l)
                address = bk.dns.inet.inet_ntop(bk.dns.inet.AF_INET6, address)
            else:
                #
