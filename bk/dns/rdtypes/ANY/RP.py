...
class RP(bk.dns.rdata.Rdata):
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin = None):
        (mbox, cused) = bk.dns.name.from_wire(wire[: current + rdlen],
                                           current)
        current += cused
        rdlen -= cused
        if rdlen <= 0:
            raise bk.dns.exception.FormError
        (txt, cused) = bk.dns.name.from_wire(wire[: current + rdlen],
                                          current)
        if cused != rdlen:
            raise bk.dns.exception.FormError
