...
class ISDN(bk.dns.rdata.Rdata):
    ...
    def to_text(self, origin=None, relativize=True, **kw):
        if self.subaddress:
            return '"%s" "%s"' % (bk.dns.rdata._escapify(self.address),
                                  bk.dns.rdata._escapify(self.subaddress))
        else:
            return '"%s"' % bk.dns.rdata._escapify(self.address)
        
    def from_text(cls, rdclass, rdtype, tok, origin = None, relativize = True):
        address = tok.get_string()
        t = tok.get()
        if t[0] != bk.dns.tokenizer.EOL and t[0] != bk.dns.tokenizer.EOF:
            tok.unget(t)
