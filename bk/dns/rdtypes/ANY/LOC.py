...
def _exponent_of(what, desc):
    ...
    if exp is None or exp < 0:
        raise bk.dns.exception.SyntaxError, "%s value out of bounds" % desc
    return exp
...
def _decode_size(what, desc):
    exponent = what & 0x0F
    if exponent > 9:
        raise bk.dns.exception.SyntaxError, "bad %s exponent" % desc
    base = (what & 0xF0) >> 4
    if base > 9:
        raise bk.dns.exception.SyntaxError, "bad %s base" % desc
    return long(base) * pow(10, exponent)

class LOC(bk.dns.rdata.Rdata):
    """LOC record
    ...
    def from_text(cls, rdclass, rdtype, tok, origin = None, relativize = True):
        ...
        if t == 'S':
            ...
            raise bk.dns.exception.SyntaxError, 'bad latitude hemisphere value'
        ...
        if t == 'W':
            ...
            raise bk.dns.exception.SyntaxError, 'bad longitude hemisphere value'
        ...
        (ttype, value) = tok.get()
        if ttype != bk.dns.tokenizer.EOL and ttype != bk.dns.tokenizer.EOF:
            if value[-1] == 'm':
                value = value[0 : -1]
            ...
            if ttype != bk.dns.tokenizer.EOL and ttype != bk.dns.tokenizer.EOF:
                ...
                if ttype != bk.dns.tokenizer.EOL and ttype != bk.dns.tokenizer.EOF:
                    if value[-1] == 'm':
                        ...
                        (ttype, value) = tok.get()
                        if ttype != bk.dns.tokenizer.EOL and \
                               ttype != bk.dns.tokenizer.EOF:
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin = None):
        ...
        if latitude < -90.0 or latitude > 90.0:
            raise bk.dns.exception.FormError, "bad latitude"
        ...
        if longitude < -180.0 or longitude > 180.0:
            raise bk.dns.exception.FormError, "bad longitude"
