...
class CERT(bk.dns.rdata.Rdata):
    ...
    def to_text(self, origin=None, relativize=True, **kw):
        ...
        return "%s %d %s %s" % (certificate_type, self.key_tag,
                                bk.dns.dnssec.algorithm_to_text(self.algorithm),
                                bk.dns.rdata._base64ify(self.certificate))
    ...
    def from_text(cls, rdclass, rdtype, tok, origin = None, relativize = True):
        ...
        if algorithm < 0 or algorithm > 255:
            raise bk.dns.exception.SyntaxError, "bad algorithm type"
        chunks = []
        while 1:
            t = tok.get()
