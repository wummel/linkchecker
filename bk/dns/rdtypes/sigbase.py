...
import bk.dns.dnssec
import bk.dns.exception
import bk.dns.rdata
import bk.dns.rdatatype

class BadSigTime(bk.dns.exception.DNSException):
...
class SIGBase(bk.dns.rdata.Rdata):
    ...
    @type signer: bk.dns.name.Name object
    ...
    def to_text(self, origin=None, relativize=True, **kw):
        return '%s %d %d %d %s %s %d %s %s' % (
            bk.dns.rdatatype.to_text(self.type_covered),
            self.algorithm,
            self.labels,
            self.original_ttl,
            ...
            self.key_tag,
            self.signer,
            bk.dns.rdata._base64ify(self.signature)
            )

    def from_text(cls, rdclass, rdtype, tok, origin = None, relativize = True):
        type_covered = bk.dns.rdatatype.from_text(tok.get_string())
        algorithm = bk.dns.dnssec.algorithm_from_text(tok.get_string())
        labels = tok.get_int()
        ...
        while 1:
            t = tok.get()
            if t[0] == bk.dns.tokenizer.EOL or t[0] == bk.dns.tokenizer.EOF:
                break
            if t[0] != bk.dns.tokenizer.IDENTIFIER:
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin = None):
        ...
        current += 18
        rdlen -= 18
        (signer, cused) = bk.dns.name.from_wire(wire[: current + rdlen], current)
        current += cused
        rdlen -= cused
