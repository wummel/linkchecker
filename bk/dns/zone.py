...
class BadZone(bk.dns.exception.DNSException):
...
class Zone(object):
    ...
    bk.dns.name.Name object, or it may be a string.  In the either case,
    ...
    node_factory = bk.dns.node.Node

    __slots__ = ['rdclass', 'origin', 'nodes', 'relativize']
    
    def __init__(self, origin, rdclass=bk.dns.rdataclass.IN, relativize=True):
    ...
    def _validate_name(self, name):
        ...
        elif not isinstance(name, bk.dns.name.Name):
            raise KeyError, \
    ...
    def get_node(self, name, create=False):
        ...
        @param create: should the node be created if it doesn't exist?
        @type create: bool
        @rtype bk.dns.node.Node object or None
        """
    ...
    def find_rdataset(self, name, rdtype, covers=bk.dns.rdatatype.NONE,
                      create=False):
        ...
        if isinstance(covers, str):
            covers = bk.dns.rdatatype.from_text(covers)
        node = self.find_node(name, create)
        return node.find_rdataset(self.rdclass, rdtype, covers, create)

    def get_rdataset(self, name, rdtype, covers=bk.dns.rdatatype.NONE,
                     create=False):
        """Look for rdata with the specified name and type in the zone,
        and return an rdataset encapsulating it.
    ...
    def delete_rdataset(self, name, rdtype, covers=bk.dns.rdatatype.NONE):
    ...
    def replace_rdataset(self, name, replacement):
        ...
        @type replacement: bk.dns.rdataset.Rdataset
        """
    ...
    def find_rrset(self, name, rdtype, covers=bk.dns.rdatatype.NONE):
        """Look for rdata with the specified name and type in the zone,
        ...
        rrset = bk.dns.rrset.RRset(name, self.rdclass, rdtype, covers)
        rrset.update(rdataset)
        return rrset

    def get_rrset(self, name, rdtype, covers=bk.dns.rdatatype.NONE):
        """Look for rdata with the specified name and type in the zone,
        and return an RRset encapsulating it.
    ...
    def iterate_rdatasets(self, rdtype=None, covers=bk.dns.rdatatype.NONE):
        """Return a generator which yields (name, rdataset) tuples for
        all rdatasets in the zone which have the specified I{rdtype}
        and I{covers}.  If I{rdtype} is bk.dns.rdatatype.ANY, the default,
        ...
        for (name, node) in self.iteritems():
            for rds in node:
                if rdtype == bk.dns.rdatatype.ANY or \
                   (rds.rdtype == rdtype and rds.covers == covers):
                    yield (name, rds)

    def iterate_rdatas(self, rdtype=None, covers=bk.dns.rdatatype.NONE):
    ...
    def check_origin(self):
        ...
        @raises bk.dns.zone.NoSOA: there is no SOA RR
        @raises bk.dns.zone.NoNS: there is no NS RRset
        @raises KeyError: there is no origin node
        """
        if self.relativize:
            name = bk.dns.name.empty
        else:
            name = self.origin
        if self.get_rdataset(name, bk.dns.rdatatype.SOA) is None:
            raise NoSOA
        if self.get_rdataset(name, bk.dns.rdatatype.NS) is None:
            raise NoNS
...
class _MasterReader(object):
    ...
    @type current_origin: bk.dns.name.Name object
    @ivar relativize: should names in the zone be relativized?
    @type relativize: bool
    @ivar zone: the zone
    @type zone: bk.dns.zone.Zone object
    ...
    def __init__(self, tok, origin, rdclass, relativize, zone_factory=Zone,
        ...
        if isinstance(origin, str):
            origin = bk.dns.name.from_text(origin)
        self.tok = tok
    ...
    def _eat_line(self):
        while 1:
            ...
            if ttype == bk.dns.tokenizer.EOL or ttype == bk.dns.tokenizer.EOF:
                break
    ...
    def _rr_line(self):
        ...
        if token[0] != bk.dns.tokenizer.WHITESPACE:
            self.last_name = bk.dns.name.from_text(token[1], self.current_origin)
        else:
            token = self.tok.get()
            if token[0] == bk.dns.tokenizer.EOL or \
               token[0] == bk.dns.tokenizer.EOF:
        ...
        # TTL
        try:
            ttl = bk.dns.ttl.from_text(token[1])
            token = self.tok.get()
            if token[0] != bk.dns.tokenizer.IDENTIFIER:
                raise bk.dns.exception.SyntaxError
        except bk.dns.ttl.BadTTL:
            ttl = self.ttl
        # Class
        try:
        ...
        if rdclass != self.zone.rdclass:
            raise bk.dns.exception.SyntaxError, "RR class is not zone's class"
        # Type
        try:
        ...
        try:
            rd = bk.dns.rdata.from_text(rdclass, rdtype, self.tok,
    ...
    def read(self):
        ...
        @raises bk.dns.zone.NoSOA: No SOA RR was found at the zone origin
        @raises bk.dns.zone.NoNS: No NS RRset was found at the zone origin
        """

        try:
            while 1:
                token = self.tok.get(True, True)
                if token[0] == bk.dns.tokenizer.EOF:
                    ...
                    if len(self.saved_state) > 0:
                        ...
                        continue
                    break
                elif token[0] == bk.dns.tokenizer.EOL:
                    continue
                elif token[0] == bk.dns.tokenizer.COMMENT:
                    self.tok.get_eol()
                    continue
                elif token[1][0] == '$':
                    u = token[1].upper()
                    if u == '$TTL':
                        token = self.tok.get()
                        if token[0] != bk.dns.tokenizer.IDENTIFIER:
                            raise bk.dns.exception.SyntaxError, "bad $TTL"
                        self.ttl = bk.dns.ttl.from_text(token[1])
                        self.tok.get_eol()
                    elif u == '$ORIGIN':
                        ...
                        token = self.tok.get()
                        if token[0] != bk.dns.tokenizer.QUOTED_STRING:
                            raise bk.dns.exception.SyntaxError, \
                                  "bad filename in $INCLUDE"
                        filename = token[1]
                        token = self.tok.get()
                        if token[0] == bk.dns.tokenizer.IDENTIFIER:
                            new_origin = bk.dns.name.from_text(token[1], \
                                                        self.current_origin)
                            self.tok.get_eol()
                        elif token[0] != bk.dns.tokenizer.EOL and \
                             token[0] != bk.dns.tokenizer.EOF:
                            raise bk.dns.exception.SyntaxError, \
                        ...
                        self.tok = bk.dns.tokenizer.Tokenizer(self.current_file,
                                                           filename)
                ...
                self.tok.unget(token)
                self._rr_line()
        except bk.dns.exception.SyntaxError, detail:
...
def from_text(text, origin, rdclass = bk.dns.rdataclass.IN, relativize = True,
    ...
    @type text: string.
    @param origin: The origin of the zone.
    @type origin: bk.dns.name.Name object or string
    @param rdclass: The zone's rdata class; the default is class IN.
    @type rdclass: int
    ...
    if filename is None:
        filename = '<string>'
    tok = bk.dns.tokenizer.Tokenizer(text, filename)
    reader = _MasterReader(tok, origin, rdclass, relativize, zone_factory,
                           allow_include=allow_include)
    reader.read()
    return reader.zone

def from_file(f, origin, rdclass = bk.dns.rdataclass.IN, relativize = True,
...
def from_xfr(xfr, zone_factory=Zone, relativize=True):
    ...
    @type xfr: generator of bk.dns.message.Message objects
    @param relativize: should names be relativized?  The default is True
    @type relativize: bool
    @raises bk.dns.zone.NoSOA: No SOA RR was found at the zone origin
    @raises bk.dns.zone.NoNS: No NS RRset was found at the zone origin
